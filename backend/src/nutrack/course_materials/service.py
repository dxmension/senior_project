...
from nutrack.course_materials.exceptions import (
    CourseMaterialForbiddenError,
    CourseMaterialNotFoundError,
    CourseMaterialQueueError,
...
def _library_response(
    entry: CourseMaterialLibraryEntry,
    download_url: str | None,
    current_user_id: int,
) -> dict:
    upload = entry.upload
...
class CourseMaterialService:
    ...
    async def queue_uploads(
        ...
        if not offering:
            raise CourseMaterialForbiddenError()
        ...
        except CourseMaterialQueueError:
    ...
    async def publish_upload(
        ...
        if not title.strip():
            raise CourseMaterialValidationError("Title is required")
        upload = await self._load_upload(upload_id)
        if upload.upload_status != MaterialUploadStatus.COMPLETED:
            raise CourseMaterialValidationError("Only completed uploads can be published")
    ...
    async def reject_upload(self, upload_id: int) -> dict:
        ...
        if upload.library_entry is not None:
            raise CourseMaterialValidationError("Published uploads cannot be rejected")
        if not _publish_requested(upload.curation_status):
            raise CourseMaterialValidationError("Private uploads cannot be rejected")
        await self.upload_repo.update(
            upload,
    ...
    async def mark_upload_failed(self, upload_id: int, message: str) -> None:
    ...
    async def process_upload(self, upload_id: int) -> None:
        ...
        if not upload.staged_path:
            raise CourseMaterialValidationError("Staged upload file is missing")
        staged_path = upload.staged_path
        await self.upload_repo.update(
            upload,
    ...
    async def _create_upload(
        ...
        if not payload:
            raise CourseMaterialValidationError("Uploaded file is empty")
        if len(payload) > MAX_FILE_SIZE:
            raise CourseMaterialValidationError("File size exceeds 25MB")
    ...
    async def _load_upload(self, upload_id: int) -> CourseMaterialUpload:
        upload = await self.upload_repo.get_upload_by_id(upload_id)
        if upload:
            return upload
        raise CourseMaterialNotFoundError()
    ...
    async def _serialize_uploads(
        self,
        uploads: list[CourseMaterialUpload | None],
    ) -> list[dict]:
        payload = []
        for upload in uploads:
            if not upload:
    ...
    async def _serialize_library(
        self,
        entries: list[CourseMaterialLibraryEntry],
        user_id: int,
    ) -> list[dict]:
        payload = []
    ...
    def _enqueue_uploads(self, uploads: list[CourseMaterialUpload]) -> None:
        from nutrack.course_materials.tasks import upload_course_material_task

        try:
            for upload in uploads:
                upload_course_material_task.delay(upload.id)
        except Exception as exc:  # pragma: no cover
            raise CourseMaterialQueueError() from exc

    def _validate_batch(self, week: int, files: list[UploadFile]) -> None:
        self._validate_week(week)
        if not files:
            raise CourseMaterialValidationError("Select at least one file")
        if len(files) > MAX_FILES_PER_BATCH:
            raise CourseMaterialValidationError("You can upload up to 10 files at once")

    def _validate_week(self, week: int) -> None:
        if 1 <= week <= 15:
            return
        raise CourseMaterialValidationError("Week must be between 1 and 15")
    ...
    def _validate_file(self, file: UploadFile) -> str:
        ...
        if filename and suffix in ALLOWED_EXTENSIONS:
            return filename
        raise CourseMaterialValidationError("Unsupported file type")
    ...
    async def _make_upload_private(self, upload: CourseMaterialUpload) -> dict:
    ...
    async def _delete_upload(self, upload: CourseMaterialUpload) -> dict:
        staged_path = upload.staged_path
    ...
    async def _delete_library_entry(self, upload: CourseMaterialUpload) -> None:
    ...
    async def _fail_stale_upload(self, upload: CourseMaterialUpload) -> None:
        staged_path = upload.staged_path
        await self.upload_repo.update(
            upload,
            upload_status=MaterialUploadStatus.FAILED,
