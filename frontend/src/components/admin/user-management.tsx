...
export function UserManagement({ users, onUpdate }: UserManagementProps) {
  ...
  const loadUserDetail = async (userId: number) => {
    try {
      const res = await api.get<ApiResponse<UserDetail>>(`/admin/users/${userId}`);
  ...
  const updateUser = async (
    userId: number,
    updates: { is_admin?: boolean; is_onboarded?: boolean }
  ) => {
    setIsUpdating(true);
    try {
      await api.patch<ApiResponse<UserDetail>>(`/admin/users/${userId}`, updates);
  ...
  const askConfirm = (opts: Omit<ConfirmState, "isOpen">) => {
              ...
              <table className="w-full">
                <thead className="border-b border-border-primary">
                  <tr>
                    ...
                    <th className="text-left py-3 px-4 text-sm font-semibold text-text-primary">Major</th>
                ...
                <tbody>
                  {displayedUsers.map((user: UserListItem) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-4 text-sm text-text-secondary">{user.major || "—"}</td>
            ...
            {selectedUser ? (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-text-primary">{selectedUser.major || "Not set"}</p>
                </div>
                <div>
                  <label className="text-xs text-text-secondary">Study Year</label>
                  <p className="text-sm text-text-primary">{selectedUser.study_year || "Not set"}</p>
                </div>
                <div>
                ...
                <div>
                  <label className="text-xs text-text-secondary">Enrollments</label>
                  <p className="text-sm text-text-primary">{selectedUser.enrollment_count} courses</p>
                </div>
                ...
                <div className="pt-4 border-t border-border-primary space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() =>
                        askConfirm({
                          title: selectedUser.is_admin ? "Revoke Admin" : "Promote to Admin",
                          message: selectedUser.is_admin
                            ? `Remove admin privileges from ${selectedUser.first_name} ${selectedUser.last_name}?`
                            : `Grant admin privileges to ${selectedUser.first_name} ${selectedUser.last_name}?`,
                          confirmLabel: selectedUser.is_admin ? "Revoke" : "Promote",
                          variant: selectedUser.is_admin ? "danger" : "default",
                          onConfirm: () =>
                            updateUser(selectedUser.id, { is_admin: !selectedUser.is_admin }),
                        })
                      }
                      disabled={isUpdating}
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-text-primary">Onboarded Status</span>
                    <button
                      onClick={() =>
                        askConfirm({
                          title: "Change Onboarding Status",
                          message: `Set ${selectedUser.first_name} ${selectedUser.last_name} as ${selectedUser.is_onboarded ? "not onboarded" : "onboarded"}?`,
                          confirmLabel: "Confirm",
                          variant: "default",
                          onConfirm: () =>
                            updateUser(selectedUser.id, { is_onboarded: !selectedUser.is_onboarded }),
                        })
                      }
                      disabled={isUpdating}
                ...
                <div className="pt-4 border-t border-border-primary">
                  <button
                    onClick={() =>
                      askConfirm({
                        title: "Remove User",
                        message: `Permanently remove ${selectedUser.first_name} ${selectedUser.last_name} (${selectedUser.email})? This action cannot be undone.`,
                        confirmLabel: "Remove",
                        variant: "danger",
