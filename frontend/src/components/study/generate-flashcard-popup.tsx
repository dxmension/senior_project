...
import { Search, X, Layers, FileText } from "lucide-react";
...
interface Props {
  ...
  onClose: () => void;
  courseId: number;
  onGenerate: (opts: GenerateFlashcardOptions) => void;
}
...
export function GenerateFlashcardPopup({
  isOpen,
  onClose,
  courseId,
  onGenerate,
  ...
  const currentItems = activeTab === "my_library" ? uploads : sharedMaterials;
  ...
  const isItemSelected = (id: number) =>
    activeTab === "my_library" ? selectedUploadIds.has(id) : selectedSharedIds.has(id);
      ...
      <div className="relative flex w-full max-w-2xl flex-col rounded-2xl border border-border-primary bg-[rgba(18,18,18,0.98)] shadow-2xl mx-4 max-h-[90vh]">
        ...
        <div className="flex flex-wrap gap-4 border-b border-border-primary px-6 py-4">
          <div className="flex flex-col gap-1">
            <label className="text-xs text-text-secondary">Cards</label>
            <input
              type="number"
              min={5}
              max={60}
              value={cardCount}
              onChange={(e) =>
                setCardCount(Math.max(5, Math.min(60, Number(e.target.value))))
              }
